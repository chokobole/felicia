...
bazel build //felicia/examples/learn/topic/python:protobuf_node_creator
```
...
```bash
bazel-bin/felicia/examples/learn/topic/python/protobuf_node_creator -p -t message
```

Running a node protobuf_node_creator in ***subscriber*** mode (without `-p` option):
```bash
bazel-bin/felicia/examples/learn/topic/python/node_creator -t message
```
