...
#include "third_party/chromium/base/feature_list.h"
...
size_t SocketBIOAdapter::GetAllocationSize() const {
  ...
  if (read_buffer_) buffer_size += read_buffer_capacity_;

  if (write_buffer_) buffer_size += write_buffer_capacity_;
  return buffer_size;
}
...
int SocketBIOAdapter::BIORead(char* out, int len) {
  ...
  if (write_error_ != net::OK && write_error_ != net::ERR_IO_PENDING &&
      (read_result_ == 0 || read_result_ == net::ERR_IO_PENDING)) {
    net::OpenSSLPutNetError(FROM_HERE, write_error_);
    return -1;
  }
  ...
  if (read_result_ == 0) {
    ...
    read_buffer_ = base::MakeRefCounted<net::IOBuffer>(read_buffer_capacity_);
    int result = socket_->Read(read_buffer_.get(), read_buffer_capacity_,
                               read_callback_);
    if (result == net::ERR_IO_PENDING) {
      read_result_ = net::ERR_IO_PENDING;
    } else {
  ...
  if (read_result_ == net::ERR_IO_PENDING) {
  ...
  if (read_result_ < 0) {
    net::OpenSSLPutNetError(FROM_HERE, read_result_);
    return -1;
  }
...
void SocketBIOAdapter::HandleSocketReadResult(int result) {
  ...
  if (read_result_ <= 0) read_buffer_ = nullptr;
}

void SocketBIOAdapter::OnSocketReadComplete(int result) {
  DCHECK_EQ(net::ERR_IO_PENDING, read_result_);
...
int SocketBIOAdapter::BIOWrite(const char* in, int len) {
  ...
  // it.
  DCHECK(write_buffer_used_ == 0 || write_error_ == net::ERR_IO_PENDING);
...
void SocketBIOAdapter::HandleSocketWriteResult(int result) {
  ...
  if (write_buffer_->RemainingCapacity() == 0) write_buffer_->set_offset(0);
  write_error_ = net::OK;

  // Release the write buffer if empty.
  if (write_buffer_used_ == 0) write_buffer_ = nullptr;
}

void SocketBIOAdapter::OnSocketWriteComplete(int result) {
  DCHECK_EQ(net::ERR_IO_PENDING, write_error_);
  ...
  if (result < 0 && read_result_ == net::ERR_IO_PENDING)
    delegate_->OnReadReady();
}

void SocketBIOAdapter::CallOnReadReady() {
  if (read_result_ == net::ERR_IO_PENDING) delegate_->OnReadReady();
}
...
SocketBIOAdapter* SocketBIOAdapter::GetAdapter(BIO* bio) {
  ...
  if (adapter) DCHECK_EQ(bio, adapter->bio());
  return adapter;
}
...
int SocketBIOAdapter::BIOWriteWrapper(BIO* bio, const char* in, int len) {
  ...
  if (!adapter) {
    net::OpenSSLPutNetError(FROM_HERE, net::ERR_UNEXPECTED);
    return -1;
  }
...
long SocketBIOAdapter::BIOCtrlWrapper(BIO* bio, int cmd, long larg,
                                      void* parg) {
  switch (cmd) {
    case BIO_CTRL_FLUSH:
