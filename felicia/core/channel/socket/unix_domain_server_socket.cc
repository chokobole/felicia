...
StatusOr<ChannelDef> UnixDomainServerSocket::BindAndListen() {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    if (!uds_endpoint.ToSockAddrStorage(&address)) {
      return errors::NetworkError(net::ErrorToString(net::ERR_ADDRESS_INVALID));
    }
...
int UnixDomainServerSocket::DoAccept() {
  int result = socket_->Accept(&accepted_socket_,
                               base::BindOnce(&UnixDomainServerSocket::OnAccept,
