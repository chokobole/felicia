...
#ifndef FELICIA_CORE_RPC_GRPC_SERVICE_IMPL_H_
#define FELICIA_CORE_RPC_GRPC_SERVICE_IMPL_H_
...
#define FEL_GRPC_SERVICE Service<T, std::enable_if_t<IsGrpcService<T>::value>>

template <typename T>
class FEL_GRPC_SERVICE {
 public:
...
template <typename T>
FEL_GRPC_SERVICE::Service(::grpc::ServerBuilder* builder)
    : is_shutdown_(false) {
...
template <typename T>
void FEL_GRPC_SERVICE::Shutdown() {
...
template <typename T>
void FEL_GRPC_SERVICE::HandleRpcsLoop() {
  typedef typename UntypedCall<FEL_GRPC_SERVICE>::Tag Tag;
  void* tag;
  bool ok;
  EnqueueRequests();
...
#endif  // FELICIA_CORE_RPC_GRPC_SERVICE_IMPL_H_