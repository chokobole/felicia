...
// https://github.com/chromium/chromium/blob/5db095c2653f332334d56ad739ae5fe1053308b1/media/capture/video/win/pin_base_win.cc
...
class TypeEnumerator final : public IEnumMediaTypes,
 ...
 public:
  ...
  STDMETHOD(Next)(ULONG count, AM_MEDIA_TYPE** types, ULONG* fetched) override {
    ...
    if (fetched) *fetched = types_fetched;
...
STDMETHODIMP PinBase::Connect(IPin* receive_pin,
  ...
  if (!receive_pin || !media_type) return E_POINTER;
...
STDMETHODIMP PinBase::ReceiveConnection(IPin* connector,
                                        const AM_MEDIA_TYPE* media_type) {
  if (!IsMediaTypeValid(media_type)) return VFW_E_TYPE_NOT_ACCEPTED;
...
STDMETHODIMP PinBase::Disconnect() {
  if (!connected_pin_.Get()) return S_FALSE;
...
STDMETHODIMP PinBase::ConnectedTo(IPin** pin) {
  *pin = connected_pin_.Get();
  if (!connected_pin_.Get()) return VFW_E_NOT_CONNECTED;
...
STDMETHODIMP PinBase::EndOfStream() { return S_OK; }

STDMETHODIMP PinBase::BeginFlush() { return S_OK; }

STDMETHODIMP PinBase::EndFlush() { return S_OK; }
...
STDMETHODIMP PinBase::ReceiveMultiple(IMediaSample** samples, long sample_count,
                                      long* processed) {
  DCHECK(samples);
...
STDMETHODIMP PinBase::ReceiveCanBlock() { return S_FALSE; }
