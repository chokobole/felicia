...
bool SinkInputPin::IsMediaTypeValid(const AM_MEDIA_TYPE* media_type) {
  ...
  if (sub_type == kMediaSubTypeI420 &&
    ...
    resulting_format_.set_pixel_format(PIXEL_FORMAT_I420);
    return true;
  }
  if (sub_type == MEDIASUBTYPE_YUY2 &&
      pvi->bmiHeader.biCompression == MAKEFOURCC('Y', 'U', 'Y', '2')) {
    resulting_format_.set_pixel_format(PIXEL_FORMAT_YUY2);
    return true;
  }
  // This format is added after http:/crbug.com/508413.
  if (sub_type == MEDIASUBTYPE_UYVY &&
      pvi->bmiHeader.biCompression == MAKEFOURCC('U', 'Y', 'V', 'Y')) {
    resulting_format_.set_pixel_format(PIXEL_FORMAT_UYVY);
    return true;
  }
  if (sub_type == MEDIASUBTYPE_MJPG &&
      pvi->bmiHeader.biCompression == MAKEFOURCC('M', 'J', 'P', 'G')) {
    resulting_format_.set_pixel_format(PIXEL_FORMAT_MJPEG);
    return true;
  }
  if (sub_type == MEDIASUBTYPE_RGB24 &&
      pvi->bmiHeader.biCompression == BI_RGB) {
    resulting_format_.set_pixel_format(PIXEL_FORMAT_BGR);
    return true;
  }
  if (sub_type == MEDIASUBTYPE_RGB32 &&
      pvi->bmiHeader.biCompression == BI_RGB) {
    resulting_format_.set_pixel_format(PIXEL_FORMAT_BGRX);
    return true;
  }
  if (sub_type == kMediaSubTypeY16 &&
      pvi->bmiHeader.biCompression == MAKEFOURCC('Y', '1', '6', ' ')) {
    resulting_format_.set_pixel_format(PIXEL_FORMAT_Y16);
    return true;
  }
  if (sub_type == kMediaSubTypeZ16 &&
      pvi->bmiHeader.biCompression == MAKEFOURCC('Z', '1', '6', ' ')) {
    resulting_format_.set_pixel_format(PIXEL_FORMAT_Z16);
    return true;
  }
...
bool SinkInputPin::GetValidMediaType(int index, AM_MEDIA_TYPE* media_type) {
  ...
  if (requested_pixel_format_ == PIXEL_FORMAT_MJPEG ||
      ...
      requested_pixel_format_ == PIXEL_FORMAT_Z16) {
    // If the requested pixel format is MJPEG, Y16 or Z16, don't accept other.
